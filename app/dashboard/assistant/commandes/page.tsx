...
import { Search, Filter, Calendar, Loader2, AlertCircle, RefreshCw } from "lucide-react"
...
export default function CommandesPage() {
  ...
  useEffect(() => {
    const fetchCommandes = async () => {
      ...
      try {
        ...
        const uniqueClients = Array.from(new Set(data.map((commande: Commande) => commande.client)))
  ...
  useEffect(() => {
    ...
    const filtered = originalCommandes.filter((commande) => {
      const matchesSearch =
        ...
        commande.adresseDestinataire.toLowerCase().includes(searchTerm.toLowerCase())

      const matchesStatus = statusFilter === "all" || commande.status === statusFilter
      const matchesClient = clientFilter === "all" || commande.client === clientFilter

      const matchesDate = !dateFilter || commande.date === format(dateFilter, "dd/MM/yyyy")
  ...
  }, [searchTerm, statusFilter, clientFilter, dateFilter, originalCommandes, itemsPerPage])
  ...
  const refreshData = async () => {
    try {
      ...
      const filtered = data.filter((commande: Commande) => {
        const matchesSearch =
          commande.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
          commande.client.toLowerCase().includes(searchTerm.toLowerCase()) ||
          commande.nom.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      setError(error instanceof Error ? error.message : "Erreur lors de l'actualisation des commandes")
      toast({
        title: "Erreur",
        description: error instanceof Error ? error.message : "Impossible d'actualiser les commandes",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  // Pagination
  const paginatedCommandes = commandesList.slice((page - 1) * itemsPerPage, page * itemsPerPage)
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="hidden xl:table-cell">Adresse départ</TableHead>
                  <TableHead className="hidden xl:table-cell">Adresse destinataire</TableHead>
                ...
                {loading ? (
                  ...
                  paginatedCommandes.map((commande) => (
                    <TableRow key={commande.id}>
                      ...
                      <TableCell className="hidden xl:table-cell max-w-[200px] truncate" title={commande.adresseDepart}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="outline" size="sm" asChild>
                          <Link href={`/dashboard/assistant/commande/${commande.id}`}>Détails</Link>
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))
