...
import { Package, Receipt, HeadphonesIcon, CheckCircle, Loader2, AlertCircle } from "lucide-react"
    ...
    if (isAuthorized) {
      const fetchDashboardData = async () => {
        try {
          ...
          const [ordersResponse, invoicesResponse, ticketsResponse] = await Promise.all([
          ...
          // Calculate stats
          const pendingOrders = ordersData.filter((order: any) => order.status === "En attente").length
          const validatedOrders = ordersData.filter((order: any) =>
            ["Validée", "Expédiée", "Livrée"].includes(order.status),
          ).length

          // Get urgent orders (pending orders)
          const urgent = ordersData
            .filter((order: any) => order.status === "En attente")
            .slice(0, 3)
            .map((order: any) => ({
              id: order.id,
              nom: order.nom,
          ...
          const pendingInvoices = invoicesData.filter(
            (invoice: any) => invoice.status === "En attente" || invoice.status === "Non envoyée",
          ).length

          // Count support tickets and get recent ones
          const supportTickets = ticketsData.filter(
            (ticket: any) => ticket.status === "Ouverte" || ticket.status === "En cours",
          ).length

          const recent = ticketsData
            .filter((ticket: any) => ticket.status === "Ouverte")
            .slice(0, 3)
            .map((ticket: any) => ({
              id: ticket.id,
              sujet: ticket.sujet,
        ...
        <TabsContent value="commandes" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Ces commandes nécessitent votre attention immédiate</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <div className="space-y-4">
                  {urgentOrders.map((order) => (
                    <div key={order.id} className="flex items-center justify-between rounded-lg border p-4">
                      ...
                      <Button asChild size="sm">
                        <Link href={`/dashboard/assistant/commandes/${order.id}`}>Voir détails</Link>
                      </Button>
                    </div>
                  ))}
                  <div className="flex justify-end">
                    <Button variant="outline" asChild>
                      <Link href="/dashboard/assistant/commandes">Voir toutes les commandes</Link>
                    </Button>
                  </div>
                </div>
              ) : (
                <p className="text-center text-muted-foreground py-4">Aucune commande en attente de validation</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="reclamations" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Les réclamations clients nécessitant votre attention</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                <div className="flex justify-center p-4">
                ...
                <div className="space-y-4">
                  {recentTickets.map((ticket) => (
                    <div key={ticket.id} className="flex items-center justify-between rounded-lg border p-4">
                      ...
                      <Button asChild size="sm">
                        <Link href={`/dashboard/assistant/reclamations/${ticket.id}`}>Voir détails</Link>
                      </Button>
                    </div>
                  ))}
                  <div className="flex justify-end">
                    <Button variant="outline" asChild>
                      <Link href="/dashboard/assistant/reclamations">Voir toutes les réclamations</Link>
                    </Button>
                  </div>
                </div>
              ) : (
                <p className="text-center text-muted-foreground py-4">Aucune réclamation récente</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
