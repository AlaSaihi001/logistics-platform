...
import { Search, Filter, Download, Send, Calendar, Loader2, AlertCircle, RefreshCw } from "lucide-react"
...
export default function FacturesPage() {
  ...
  useEffect(() => {
    const fetchFactures = async () => {
      ...
      try {
        ...
        const uniqueClients = Array.from(new Set(data.map((facture: Facture) => facture.client)))
        ...
        toast({
          title: "Erreur",
          description: error instanceof Error ? error.message : "Impossible de charger les factures",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
  ...
  useEffect(() => {
    ...
    const filtered = originalFactures.filter((facture) => {
      ...
      const matchesClient = clientFilter === "all" || facture.client === clientFilter

      const matchesDate = !dateFilter || facture.issueDate === format(dateFilter, "dd/MM/yyyy")
  ...
  }, [searchTerm, statusFilter, clientFilter, dateFilter, originalFactures, itemsPerPage])
  ...
  const refreshData = async () => {
    try {
      ...
      setError(error instanceof Error ? error.message : "Erreur lors de l'actualisation des factures")
      toast({
        title: "Erreur",
        description: error instanceof Error ? error.message : "Impossible d'actualiser les factures",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const handleSendInvoice = async (id: string) => {
    try {
      ...
      // Update local state
      setFacturesList(facturesList.map((facture) => (facture.id === id ? { ...facture, status: "envoyee" } : facture)))
      ...
      toast({
        title: "Erreur",
        description: error instanceof Error ? error.message : "Impossible d'envoyer la facture",
        variant: "destructive",
      })
    } finally {
      setSubmitting(false)
  ...
  // Pagination
  const paginatedFactures = facturesList.slice((page - 1) * itemsPerPage, page * itemsPerPage)
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Gérez les factures des commandes</p>
        </div>
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="non-envoyee">Non envoyée</SelectItem>
                ...
                {loading ? (
                  ...
                  paginatedFactures.map((facture) => (
                    <TableRow key={facture.id}>
                      ...
                      <TableCell>
                        <Link
                          href={`/dashboard/assistant/commande/${facture.commande}`}
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2 flex-wrap">
                          <Button variant="outline" size="sm" asChild>
                            <Link href={`/dashboard/assistant/factures/${facture.id}`}>Voir</Link>
                          </Button>
                          <Button variant="outline" size="sm" className="gap-1">
                            <Download className="h-4 w-4" />
                            <span className="hidden sm:inline">PDF</span>
                          </Button>
                          {facture.status === "non-envoyee" && (
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Êtes-vous sûr de vouloir envoyer la facture {facture.id} au client {facture.client}{" "}
                                    ?
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
