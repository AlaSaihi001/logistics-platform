...
interface Claim {
  ...
  client: { nom: string; prenom: string } | null; // Ensure client is nullable initially
}

export default function ClaimDetailPage() {
  const { id } = useParams(); // Get the dynamic ID using useParams
  ...
  useEffect(() => {
    const fetchClaim = async () => {
      ...
      try {
        ...
        toast({
          title: "Error",
          description: "Unable to fetch claim details",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
