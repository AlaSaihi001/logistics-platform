...
import { getReclamationDetails, updateReclamationStatus } from "@/lib/assistant-service"
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(
      { error: "Une erreur est survenue lors de la récupération des détails de la réclamation" },
      { status: 500 }
    )
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await updateReclamationStatus(reclamationId, assistantId, status)

    return NextResponse.json(result)
  } catch (error) {
    console.error("Error updating reclamation:", error)
    return NextResponse.json(
      { error: "Une erreur est survenue lors de la mise à jour de la réclamation" },
      { status: 500 }
    )
  }
}
