...
export async function POST(req: NextRequest) {
  try {
    ...
    else if (!/^\d+$/.test(telephone)) errors.telephone = "Le numéro de téléphone doit être composé uniquement de chiffres"

    if (!indicatifTelephone) errors.indicatifTelephone = "L'indicatif téléphonique est requis"
    ...
    try {
      console.log("🚀 Tentative d'enregistrement avec :", {
        email, nom, prenom, telephone, indicatifTelephone
      })
      ...
      return ApiResponse.serverError("Erreur interne lors de la création du compte")
    }
  } catch (error) {
