...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!result.success) {
      return ApiResponse.error(result.error ?? "Erreur lors de la récupération des factures", { status: 400 })
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    return ApiResponse.success({ message: "Facture créée avec succès" }, { status: 201 })
  } catch (error) {
