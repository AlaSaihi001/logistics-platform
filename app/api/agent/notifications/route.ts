...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!result.success) {
      return ApiResponse.error(result.error ?? "Erreur lors de la récupération des notifications", { status: 400 })
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!result.success) {
      return ApiResponse.error(result.error ?? "Impossible de marquer la notification comme lue", { status: 400 })
    }
